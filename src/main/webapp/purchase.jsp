...
function gopage(nowpage){
	...
	if(parseInt(nowpage)<=0){
		...
		splitfrom.cp.value = nowpage;
		splitfrom.submit();
	}
}
function gosearch(){
	var nowpage = parseInt(document.getElementById("cp").value);
	var totalpage = parseInt(document.getElementById("totalpage").value);
...
//全部采购
function enter(){
	location.href = "pc/enter"
}

//正在进行的采购
function conduct(){
	location.href = "pc/conduct?purchase_state=1"
}

//已结束的采购
function closure(){
	location.href = "pc/closure?purchase_state=2"
}
</script>
</head>
<body>
	...
	<div class="content">
		<div class="container">
			...
			<div class="content-main">
				...
				<div class="lib-tab4">
					<ul class="lib-menu">
						<li class="hover" onclick="enter();">全部采购</li>
						<li onclick="conduct();">正在进行采购</li>
						<li onclick="closure();">已结束采购</li>
					</ul>
					<div class="lib-content">
						<div class="child-content" style="display: block;">
						<form action="pc/enter" name="splitfrom" method="post">
							...
							<div class="purchase-left">
								...
								<c:forEach items="${purchaselist }" var="pl">
									<dd>
										...
										<div class="tit">
											<a href="pc/findpurchasebyid?purchaseid=${pl.purchaseid }" class="otw">${pl.purchase_title }</a>
											<span class="goOn">
											【<c:choose>
												<c:when test="${pl.purchase_state==0 }">待审核</c:when>
												<c:when test="${pl.purchase_state==1 }">进行中</c:when>
												<c:when test="${pl.purchase_state==2 }">已完成</c:when>
												<c:when test="${pl.purchase_state==3 }">已过期</c:when>
											</c:choose>】</span>
										</div>
										<div class="user">${fn:substring(pl.contacts,0,1) }${fn:length(pl.contacts)>1?"**":"" }&nbsp;&nbsp;&nbsp;于<fmt:formatDate value="${pl.start_time }" pattern="yyyy-MM-dd HH:mm:ss"/>发表</div>
										<div class="intro">${fn:substring(pl.purchase_description,0,15) }${fn:length(pl.purchase_description)>15?"...":"" }</div>
									</dd>
								</c:forEach>
								</dl>
								<!-- page -->
								...
								<input type="hidden" value="${purchase_state }" name="purchase_state" id="purchase_state"/>
								<div class="list-page">
									<ul>
										<li><a href="javascript:;" class="prev-listPage " onclick="gopage('${cp-1}');">上一页</a></li>
										...
										<li><a href="javascript:;" class="next-listPage" onclick="gopage('${cp+1}');">下一页</a></li>
										<li><span>到第</span><input type="text" value="" name="cp" id="cp" ><span>页</span></li>
										<li><a href="javascript:;" class="jump-page" onclick="gosearch();">确定</a></li>
									</ul>
								</div>
							</div>
							</form>
							...
							<div class="purchase-right">
								<!-- 发布清单按钮 -->
								<a href="pc/release" class="btn05"><i
									class="fa fa-file-text-o"></i>我要发布采购清单</a>
								<!-- 热门采购单 -->
								<div class="inventory">
								...
								<!-- 资源广告 -->
								<a href="javascript:;" class="purchase-add"><img
									src="images/purchase-add01.jpg" alt=""></a>
							</div>
						</div>
					</div>
				</div>
