            ...
            <div class="member-tit">
             ...
             <div class="proof">
                 <div class="member-column">
                     <div class="member-detailtit">${purchase.purchase_title }</div>
                     <div class="detail-con">
                         <dl>
                            <dt>产品：</dt>
                            <dd>${purchase.purchase_name }</dd>
                            <dt>产地要求：</dt>
                            <dd>${purchase.producing_area }</dd>
                            <dt>规格型号：</dt>
                            <dd>${purchase.purchase_type }</dd>
                            <dt>级别要求：</dt>
                            <dd>${purchase.purchase_level }</dd>
                            <dt>仓库要求：</dt>
                            <dd>${purchase.storehouse }</dd>
                            <dt>期/现货：</dt>
                            <dd>
                            <c:choose>
                            	<c:when test="${purchase.state==0 }">期货</c:when>
                            	<c:when test="${purchase.state==1 }">现货</c:when>
                            	<c:when test="${purchase.state==2 }">不限</c:when>
                            </c:choose>
                            </dd>
                            <dt>价格要求：</dt>
                            <dd>${purchase.price_require }元/千克</dd>
                            <dt>采购说明：</dt>
                            <dd>${purchase.purchase_description }</dd>
                            <dt>付款方式：</dt>
                            <dd>${purchase.payment_method }</dd>
                            <dt>发票类型：</dt>
                            <dd>
                            <c:choose>
                            	<c:when test="${purchase.invoice_type==0 }">增值税发票</c:when>
                            	<c:when test="${purchase.invoice_type==1 }">普通发票</c:when>
                            	<c:when test="${purchase.invoice_type==2 }">其他</c:when>
                            </c:choose>
                            </dd>
                            <dt>收货地址：</dt>
                            <dd>${purchase.receipt_address }</dd>
                            <dt>采购截止时间：</dt>
                            <dd>
                            <fmt:formatDate value="${purchase.end_time }" pattern="yyyy-MM-dd"/>
                            </dd>
                         </dl>
                     </div>
                </div>
                    ...
                    <div class="cg-contact">
                        ...
                        <div class="detail-con">
                            ...
                            <dl>
                                <dt>联系人：</dt>
                                <dd><p>
                                ${fn:substring(purchase.contacts,0,1) }${fn:length(purchase.contacts)>1?"**":"" }
                                </p></dd>
                                <dt>公司名称：</dt>
                                <dd>
                                    <p>
                                    ${fn:substring(purchase.company_name,0,2) }${fn:length(purchase.company_name)>2?"****":"" }${fn:substring(purchase.company_name,fn:length(purchase.company_name)-2,fn:length(purchase.company_name)) }
                                    </p>
                                </dd>
                                <dt>手机号：</dt>
                                <dd>
                                    <p> ${fn:substring(purchase.contacts_phone,0,3) }${fn:length(purchase.company_name)>3?"****":"" }${fn:substring(purchase.contacts_phone,fn:length(purchase.contacts_phone)-4,fn:length(purchase.contacts_phone)) }</p>
                                </dd>
                            </dl>
                        </div>
                    </div>
